    ...
    public void initializeForeignCache() throws IllegalAccessException {
        ...
        for (Class<?> p : ps) {
            ...
            if (null!=map){
                ...
                BaseMapper baseMapper = springUtils.getBean(StringUtils.underlineToCamel(table) + "Mapper");
                List<Map<String,Object>> list = baseMapper.selectMaps(new QueryWrapper().select(StringUtils.camelToUnderline(idField)+" as value",StringUtils.camelToUnderline(nameField)+" as label"));
